...
class PostRecipeSerializer(serializers.ModelSerializer):
    ingredients = PostRecipeIngredientSerializer(many=True, source='ingredients_in')
    ...
    def update(self, instance, validated_data):
        if 'ingredients' in self.initial_data:
            ...
            for ingredient in ingredients:
                ...
                if RecipeIngredient.objects.filter(recipe=instance,ingredient=current_ingredient).exists():
                    instance.ingredient.amount = ingredient['amount']
                else:
                    RecipeIngredient.objects.update_or_create(recipe=instance, 
                                                              ingredient=current_ingredient, 
                                                              amount=ingredient['amount'])
...
class ShoppingListSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        user = data['user']
        if Favourite.objects.filter(recipe=recipe_id, user=user).exists():
            raise serializers.ValidationError('This recipe is already in your shopping list!')
        return data
